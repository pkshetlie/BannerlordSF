...
class ApiController extends AbstractController
{
    ...
    public function api(Request $request, string $apiKey, RunService $runService)
    {
        ...
        $run = $entityManager->getRepository(Run::class)
            ...
            ->leftJoin("challenge.participations", 'participations')
            ->leftJoin("participations.user", 'user')
            ->where('user.apiKey = :apikey')
            ->andWhere('((challenge_dates.startDate <= :now AND challenge_dates.endDate >= :now) OR (participations.openChallenge <= :now AND participations.closeChallenge >= :now))')
